          ...
          data.entry.forEach((entry) =>  {
            entry.messaging.forEach((event) => {
              ...
              if (event.sender && event.sender.id) {
                ...
                const tokenParam = config.access_token ? `?access_token=${config.access_token}` : ''
          ...
          data.entry.forEach((entry) => {
            ...
            entry.messaging.forEach((event) => {
              ...
              if (event.postback && event.postback.referral) {
                request.refParams = internals.trParamToParamList(event.postback.referral.ref)
...
internals.strParamToParamList = (strParam) => {
  ...
  const params = strParamsList.map((param, index) => {
    const obj = {}

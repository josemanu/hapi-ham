          ...
          data.entry.forEach(function (entry) {
            entry.messaging.forEach(function (event) {
              ...
              if (event.postback && event.postback.referral) {
                response.refParams = strParamToParamList(event.postback.referral)
              } else if (event.referral) {
                response.refParams = strParamToParamList(event.referral)
              }
            })
          })
        }
...
function strParamToParamList(strParam) {
  let strParams = event.postback.referral.split(',')
  return strParams.map((param, index) => {
    const obj = {}

          ...
          data.entry.forEach(function (entry) {
            entry.messaging.forEach(function (event) {
              ...
              if (event.postback && event.postback.referral) {
                response.refParams = strParamToParamList(event.postback.referral.ref)
              } else if (event.referral) {
                response.refParams = strParamToParamList(event.referral.ref)
              }
            })
          })
        }
...
function strParamToParamList(strParam) {
  ...
  return strParamsList.map((param, index) => {
    const obj = {}
